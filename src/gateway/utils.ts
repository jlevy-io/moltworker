 ...
 *
 * Accepts a single sentinel string or an array. Returns `found: true` when
 * any sentinel is detected, so the caller can inspect stdout to determine
 * which one matched (e.g. SYNC_OK vs SYNC_FAIL).
 */
