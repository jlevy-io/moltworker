...
export async function syncToR2(sandbox: Sandbox, env: MoltbotEnv): Promise<SyncResult> {
  ...
  const syncCmd = `rsync -r --no-times --delete --exclude='*.lock' --exclude='*.log' --exclude='*.tmp' /root/.clawdbot/ ${R2_MOUNT_PATH}/clawdbot/ && rsync -r --no-times --delete /root/clawd/skills/ ${R2_MOUNT_PATH}/skills/ && ([ -d /root/.config/gogcli ] && rsync -r --no-times /root/.config/gogcli/ ${R2_MOUNT_PATH}/gogcli/ || true) && date -Iseconds > ${R2_MOUNT_PATH}/.last-sync`;
