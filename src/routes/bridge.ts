...
bridge.get('/', async (c) => {
  ...
  console.log(
    '[BRIDGE] WebSocket upgrade request, rewritten to',
    gatewayUrl.pathname + gatewayUrl.search.replace(/token=[^&]+/, 'token=***'),
  );

  // Ensure gateway is running before connecting
  try {
  ...
  console.log(
    '[BRIDGE] wsConnect returned, status:',
    containerResponse.status,
    'hasWebSocket:',
    !!containerResponse.webSocket,
  );
  ...
  if (!containerWs) {
    ...
    console.error(
      '[BRIDGE] No WebSocket in container response, status:',
      containerResponse.status,
      'body:',
      body,
    );
    return c.json(
      { error: 'Gateway WebSocket unavailable', status: containerResponse.status, body },
      502,
    );
  }
...
bridge.get('/health', async (c) => {
  ...
  const gateway = processes.find(
    (p: { command: string; status: string }) =>
      p.command.includes('start-openclaw') && p.status === 'running',
  );
